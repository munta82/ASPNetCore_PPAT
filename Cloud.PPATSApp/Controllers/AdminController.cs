...
namespace Cloud.PPATSApp.Controllers
{
    public class AdminController : Controller
    {
        ...
        public IActionResult DownloadEmployeeReport(string id)
        {

            //databaseAPI.DownloadExcelDocument(id);
            DataTable dataTable = new DataTable();
            List<EmployeeInfoViewModel> lstUsers = new List<EmployeeInfoViewModel>();
            DataSet dsEmp = new DataSet();
            CloudPATContext dbContext = new CloudPATContext();
            var dbConnection = dbContext.Database.GetDbConnection().ConnectionString;
            ...
            using (var con = new SqlConnection(dbConnection))
            {
                using (SqlCommand cmd = new SqlCommand("prcExportEmployeeDataToExcel", con))
                {
                    using (var da = new SqlDataAdapter(cmd))
                    {
                        cmd.CommandType = System.Data.CommandType.StoredProcedure;
            ...
            lstUsers = CommonExtensions.CreateListFromTable<EmployeeInfoViewModel>(dsEmp.Tables[0]);
            ...
            try
            {
                using (var workbook = new XLWorkbook())
                {
                    IXLWorksheet worksheet =
                    workbook.Worksheets.Add("Authors");
                    worksheet.Cell(1, 1).Value = "EmpId";
                    worksheet.Cell(1, 2).Value = "EmpFirstName";
                    worksheet.Cell(1, 3).Value = "EmpLastName";
                    worksheet.Cell(1, 4).Value = "EmpPhone";
                    worksheet.Cell(1, 5).Value = "EmpEmail";
                    worksheet.Cell(1, 6).Value = "EmpAddress";
                    worksheet.Cell(1, 7).Value = "EmpUsername";
                    worksheet.Cell(1, 8).Value = "EmpPassword";
                    worksheet.Cell(1, 9).Value = "EmpSecureId";
                    worksheet.Cell(1, 10).Value = "IsActive";
                    worksheet.Cell(1, 11).Value = "CreatedDate";
                    worksheet.Cell(1, 12).Value = "CreatedBy";
                    worksheet.Cell(1, 13).Value = "ModifiedDate";
                    worksheet.Cell(1, 14).Value = "ModifiedBy";
                    worksheet.Cell(1, 15).Value = "EmpRoleId";
                    worksheet.Cell(1, 16).Value = "empApps";
                    

                    for (int index = 1; index <= lstUsers.Count; index++)
                    {
                        worksheet.Cell(index + 1, 1).Value = lstUsers[index - 1].EmpId;
                        worksheet.Cell(index + 1, 2).Value = lstUsers[index - 1].EmpFirstName;
                        worksheet.Cell(index + 1, 3).Value = lstUsers[index - 1].EmpLastName;
                        worksheet.Cell(index + 1, 4).Value = lstUsers[index - 1].EmpPhone;
                        worksheet.Cell(index + 1, 5).Value = lstUsers[index - 1].EmpEmail;
                        worksheet.Cell(index + 1, 6).Value = lstUsers[index - 1].EmpAddress;
                        worksheet.Cell(index + 1, 7).Value = lstUsers[index - 1].EmpUsername;
                        worksheet.Cell(index + 1, 8).Value = lstUsers[index - 1].EmpPassword;
                        worksheet.Cell(index + 1, 9).Value = lstUsers[index - 1].EmpSecureId;
                        worksheet.Cell(index + 1, 10).Value = lstUsers[index - 1].IsActive;
                        worksheet.Cell(index + 1, 11).Value = lstUsers[index - 1].CreatedDate;
                        worksheet.Cell(index + 1, 12).Value = lstUsers[index - 1].CreatedBy;
                        worksheet.Cell(index + 1, 13).Value = lstUsers[index - 1].ModifiedDate;
                        worksheet.Cell(index + 1, 14).Value = lstUsers[index - 1].ModifiedBy;
                        worksheet.Cell(index + 1, 15).Value = lstUsers[index - 1].EmpRoleId;
                        worksheet.Cell(index + 1, 16).Value = lstUsers[index - 1].empApps;
